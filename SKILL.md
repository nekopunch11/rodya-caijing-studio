...
5. **渲染**：按下方默认产出规则；单模块默认生成专业研究 `.docx`（按 [references/docx-visual-spec.md](references/docx-visual-spec.md)）。DOCX 统一使用唯一生成链 `scripts/docx_renderer.py`，从 `assets/docx-template.docx` 起稿并读取 `references/docx-style-tokens.json`；各模块只提供结构化内容块，禁止自行创建 `Document()` 或硬编码字体、颜色、字号、页边距、表格宽度和首页版式。每次生成后运行 `scripts/audit_docx_visual.py`，只检查 DOCX 文件完整性、文档结构、章节层级、样式参数、核心元信息和必要内容；不执行 render、不生成 PNG、不进行逐页视觉审核。**财经内容台 DOCX 不得调用通用 `documents` skill 的 `render_docx.py`、LibreOffice/`soffice` 或其 PNG 门禁；若通用文档 skill 被自动匹配，以本项目流程为准。**审计通过即可标记为“通过 DOCX 结构与样式审计”；审计失败仍可输出 DOCX，但必须列出失败项并标注“未通过审计”，不得声称其已符合视觉规范；只有文件损坏、无法打开或生成失败时，才不得作为有效 DOCX 交付。客户合规口径与交付形态彼此独立：客户版可以是 docx、卡片+文案或 PPT；三形态均消费同一内核。PPT 受众沿用用户指定口径，未说明时使用专业研究口径。
   - **子 Skill 继承边界**：七个 `caijing-*` 子 Skill 继承本父层 DOCX 工作流和 [references/docx-visual-spec.md](references/docx-visual-spec.md)，只定义本模块的内容结构、研究方法和领域检查；不得重新指定 renderer、模板、style tokens、字体、颜色、字号、页边距、表格宽度、首页版式或 DOCX 的 render/PNG/逐页审核流程。若子 Skill 与父层或视觉规范冲突，以父层和视觉规范为准。
...
7. **交付**：每份输出标注数据截至日 + 确定性分级 + 免责声明。客户卡按执行端分流：**ChatGPT Codex** 交付由同一 HTML 实际捕获的 PNG + HTML 源文件 + 文案；**Claude** 交付 HTML 源文件 + 文案，并明确提醒使用者在浏览器打开后截图。若 Codex 的实际捕获能力受环境阻断，必须报告阻断并按 HTML + 截图提醒降级，严禁以生成式示意图、旧图或不一致画面冒充 PNG。**交付专业版后主动提醒使用者：如需转客户，可再让我出客户版（卡片图+文案）。**
...
| [card-components.md](references/card-components.md) | 客户卡片唯一视觉事实源：墨账本 Swiss、数据台密度、七模块签名图形与 Codex PNG / Claude HTML 交付规则 |
