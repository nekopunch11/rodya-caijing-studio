...
3. 取数（财报原文优先），逐条标来源与确定性；没有本期财报原文时按 data-fallback.md 降级，不做超预期判定。
4. 依七个分析块产出结构化内核（一份，唯一事实源）。
5. 渲染专业版 docx + 客户版卡片文案（口径切换见 output-spec.md 与 compliance-rendering.md）。
6. 合规检查（客户版逐句过 checklist；禁"会涨/会跌"类引申）。
7. 交付，标数据截至日 + 确定性分级 + 数据缺口。
